...
analyze_counts_file <- function(counts_file, fasta_file) {
  ...
  # Remove replicate suffixes, e.g. "_1", "_2"
  ...
  output_dir <- paste0(species_id, "_analysis_results")
  ...
  # --- CREATE MATRICES ---
  species_up_matrix <- matrix(0, nrow = n_tissues, ncol = n_tissues,
                              dimnames = list(tissues, tissues))
  species_down_matrix <- matrix(0, nrow = n_tissues, ncol = n_tissues,
                                dimnames = list(tissues, tissues))
  ...
  for (comparison in names(pairwise_species_up)) {
    ...
    species_up_matrix[t1, t2] <- up_count
    species_up_matrix[t2, t1] <- down_count
    
    species_down_matrix[t1, t2] <- down_count
    species_down_matrix[t2, t1] <- up_count
  }
  
  write.csv(species_up_matrix, file.path(output_dir, paste0(species_id, "_SPL_upregulated_gene_matrix.csv")))
  write.csv(species_down_matrix, file.path(output_dir, paste0(species_id, "_SPL_downregulated_gene_matrix.csv")))
  
  cat("\nMatrix of UPREGULATED", species_id, "SPL genes between tissues:\n")
  ...
  species_plot <- ggplot(species_melted, aes(x = Comparison, y = SPL_Gene_Count, fill = Direction)) +
    ...
    labs(
      title = paste0(species_id, " SPL genes up- and downregulated in pairwise tissue comparisons"),
      x = "Tissue Comparison",
  ...
  ggsave(file.path(output_dir, paste0(species_id, "_SPL_summary_plot.png")), species_plot, 
